        ...
        try:
            if settings['is_shortlink'] and not await db.has_premium_access(query.from_user.id):
                if clicked == typed:
                    ...
                    await query.answer(url=f"{BLOGSPOT_URL}?url=https://telegram.me/{temp.U_NAME}?start=short_{file_id}")
                    return
                else:
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        ...
        fuk = await reply_msg.edit_text(text=cap, reply_markup=InlineKeyboardMarkup(btn) if btn else None, disable_web_page_preview=True)   
        try:
            if settings['auto_delete']:
                await asyncio.sleep(300)
                await fuk.delete()
