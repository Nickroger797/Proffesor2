...
import urllib.parse
from info import BLOGSPOT_URL  # Make sure only this is imported
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if imdb:
        ...
        BLOGSPOT_URL = "https://technoji786.blogspot.com/2025/02/codex-channel.html?next="  # Blogspot redirect page
        ...
        if not settings["button"]:
            ...
            for file in files:
                ...
                blogspot_redirect = BLOGSPOT_URL + telegram_link  # Redirect via Blogspot
                cap += f"<b>\n📁 <a href='{blogspot_redirect}'>[{get_size(file['file_size'])}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file['file_name'].split()))}\n</a></b>"
    else:
        if settings["button"]:
